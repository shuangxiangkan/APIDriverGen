...
from LLM.prompts import get_APIs_with_options_prompt, generate_fuzz_driver_afl_prompt
...
def get_fuzz_driver(api):
    ...
    print(f"{ColorPrint.red(param_types_and_return_type)}")
    print(f"{ColorPrint.blue(api_signature)}")
    print(f"{ColorPrint.green(comments)}")
    print(f"{ColorPrint.red(usage_examples)}")
    print(f"{ColorPrint.blue(additional_info)}")


    fuzz_driver_prompt = generate_fuzz_driver_afl_prompt(param_types_and_return_type, api_signature, comments, usage_examples, additional_info)
    # # print (fuzz_driver_prompt)
    # #
    gpt_response = call_gpt(fuzz_driver_prompt)
    # # # print(gpt_response)
    # #
    parse_results = parse_driver_response(gpt_response)
    # print(f"{ColorPrint.red(parse_results)}")
...
if __name__ == "__main__":
    library_path = "/Users/shuangxiangkan/Code/PycharmProject/APIDriverGen/Libraries/yyjson"
    header_paths = ["/Users/shuangxiangkan/Code/PycharmProject/APIDriverGen/Libraries/yyjson/src/yyjson.h"]

    analyzer = CLibraryAnalyzer(library_path, header_paths, api_prefix='yyjson_')
    # analyzer.parse_headers()
    analyzer.parse()

    api = analyzer.get_api_by_name("yyjson_write_fp")
    ...
    #
    # api_comments_and_sigs = get_all_api_comments_and_signatures(all_apis)
    # # print(api_comments_and_sigs)
    #
    # get_APIs_with_options(api_comments_and_sigs)
